...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix btccore to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new btccore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in btccore-node.
      var header = btccore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [btccore.Transaction().fromBufferReader(br)];
